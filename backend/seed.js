...
const categoriesData = [
  { name: 'Rice & Grains', slug: 'rice-grains', icon: '🍚', image: images.rice, description: 'Premium quality rice and grains for your daily meals', order: 1 },
  { name: 'Atta & Flour', slug: 'atta-flour', icon: '🌾', image: images.flour, description: 'Fresh whole wheat atta and specialty flours', order: 2 },
  { name: 'Pulses & Dal', slug: 'pulses-dal', icon: '🫘', image: images.dal, description: 'Nutritious pulses and dal varieties', order: 3 },
  { name: 'Cooking Oils', slug: 'cooking-oils', icon: '🫒', image: images.oil, description: 'Pure and refined cooking oils', order: 4 },
  { name: 'Spices & Masala', slug: 'spices-masala', icon: '🌶️', image: images.spices, description: 'Authentic Indian spices and masala blends', order: 5 },
  { name: 'Salt & Sugar', slug: 'salt-sugar', icon: '🧂', image: images.salt, description: 'Essential salt and sugar products', order: 6 },
  { name: 'Tea & Coffee', slug: 'tea-coffee', icon: '☕', image: images.tea, description: 'Premium tea leaves and coffee blends', order: 7 },
  { name: 'Biscuits & Cookies', slug: 'biscuits-cookies', icon: '🍪', image: images.biscuits, description: 'Delicious biscuits and cookies for tea time', order: 8 },
  { name: 'Snacks & Namkeen', slug: 'snacks-namkeen', icon: '🍿', image: images.snacks, description: 'Crunchy snacks and traditional namkeen', order: 9 },
  { name: 'Beverages', slug: 'beverages', icon: '🥤', image: images.beverages, description: 'Refreshing drinks and beverages', order: 10 },
  { name: 'Dairy Products', slug: 'dairy-products', icon: '🥛', image: images.dairy, description: 'Fresh dairy products delivered daily', order: 11 },
  { name: 'Bread & Bakery', slug: 'bread-bakery', icon: '🍞', image: images.bread, description: 'Freshly baked bread and bakery items', order: 12 },
  { name: 'Fruits & Vegetables', slug: 'fruits-vegetables', icon: '🥬', image: images.vegetables, description: 'Farm-fresh fruits and vegetables', order: 13 },
  { name: 'Cleaning & Household', slug: 'cleaning-household', icon: '🧹', image: images.cleaning, description: 'Cleaning supplies and household essentials', order: 14 },
  { name: 'Personal Care', slug: 'personal-care', icon: '🧴', image: images.personalCare, description: 'Personal hygiene and care products', order: 15 },
];
...
const seedDatabase = async () => {
  try {
    ...
    coupons.forEach((c) => {
      ...
      const min = c.minOrderAmount > 0 ? `, min order ₹${c.minOrderAmount}` : '';
      console.log(`   - ${c.code}: ${desc}${min}`);
    });

    // Summary
    console.log('\n' + '═'.repeat(50));
    console.log('🏪 RADHAKRISHNA GENERAL STORE — SEED COMPLETE');
    ...
    console.log(`   🚫 Out of Stock: ${await Product.countDocuments({ stock: 0 })}`);
    console.log(`   ⚠️  Low Stock:   ${await Product.countDocuments({ stock: { $gt: 0, $lte: 10 } })}`);
    console.log('═'.repeat(50));
